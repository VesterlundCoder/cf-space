...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{fig2_commutator_heatmap}
\caption{Commutator norm matrix $\|[T,S]\|$ on a log scale. The only
large off-diagonal values involve \texttt{sign\_flip\_b} and
\texttt{shift}/\texttt{backshift} (values~4.06--4.50), highlighted in dark
orange.  All other pairs commute to within numerical precision.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{fig3_manifold_closure}
\caption{Manifold closure: mean distance from $E(T(x))$ to the nearest
corpus point, expressed as a ratio over the baseline (identity).  Bars
are coloured by severity: grey~=~on manifold ($<2\times$), blue~=~slightly
off ($2$--$10\times$), red~=~off manifold ($>10\times$).  \texttt{shift}
at $1075\times$ is truncated for readability.}
\label{fig:closure}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{fig4_lipschitz}
\caption{Lipschitz constants per operator: mean~(blue bars), median~(white
circles), and max~(red triangles).  All operators except \texttt{shift} have
max~$L < 0.05$; the \texttt{shift} outlier at max~$L = 11.76$ is annotated.}
\label{fig:lipschitz}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{fig5_conservation}
\caption{Conservation rates per operator for four structural properties.
Scale and sign-flip operators are fully conservative (all 100\%).  Ap\'{e}ry
operators break the degree of $a(n)$ (36\%) but preserve all other properties.
\texttt{shift} breaks both deg$(a)$ (29\%) and sign$(a_0)$ (39\%).  All
operators preserve the norm class (100\%, purple bars all at ceiling).}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig6_law_summary}
\caption{Algebraic law pass/fail summary across all 10 operators and 11
selected laws.  Green~(+)~=~passes, amber~(\textasciitilde)~=~approximate
pass, red~($-$)~=~fails.  The smooth subset (scale, apery, sign-flip) shows
broad green coverage.  \texttt{shift} and \texttt{random\_sm} fail most laws.
The identity operator serves as ground-truth reference.}
\label{fig:summary}
\end{figure}
